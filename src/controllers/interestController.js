...
class InterestController {
  ...
  static async removeInterest(req, res) {
    try {
      ...
      const removedInterest = await pool.query(
        "DELETE FROM interests WHERE interest = $1 AND user_id = $2 RETURNING *",
        [interest, req.user]
      );
  ...
  static async setOnboarding(req, res) {
    try {
      await pool.query("UPDATE users SET onboarding = false WHERE id = $1", [
        req.user,
      ]);
